...
def convert_data_units(data: np.ndarray, scale:int) -> np.ndarray:
    """Convert data based on units."""

    return data / 10**scale

def get_optimal_data_and_unit(data, unit):
    opt_scale = find_scale_and_prefix(data, unit)
    opt_unit = x_opt_unit = opt_scale[0] + unit
    opt_data = convert_data_units(data, opt_scale[1])
    return opt_data, opt_unit


def get_title_string(dataset):
    # Extracting the file name without extension
    file_name = os.path.splitext(os.path.basename(dataset.path_to_db))[0]
    # Extracting the last name (assuming it is the part after the last underscore)
    databse_name = file_name.split('\\')[-1]
    run_id = str(dataset.run_id)
    sameple_name = dataset.sample_name
    exp_name = dataset.exp_name
    measurement_name = dataset.name

    return f"{databse_name}.db | run ID: {run_id}\n{sameple_name} | {exp_name} | {measurement_name}"
